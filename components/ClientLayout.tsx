  ...
  useEffect(() => {
    ...
    if (sessionTimestamp) {
      ...
      if (isNaN(timestamp) || (now - timestamp) > SESSION_DURATION) {
        ...
        console.warn('Session expired! Clearing user data for security.');
        logout();
      ...
      console.warn('Invalid session format! Clearing user data for security.');
      logout();
      localStorage.removeItem('current-user');
      localStorage.removeItem('orders');
      localStorage.removeItem('saved-addresses');
    ...
    if (storedUser) {
      try {
        ...
        if (currentUser) {
          ...
          if (currentUser.email !== parsedUser.email || currentUser.id !== parsedUser.id) {
            console.warn('User session mismatch detected! Clearing session for security.');
            logout();
            localStorage.removeItem('current-user');
            localStorage.removeItem('user-session-timestamp');
            localStorage.removeItem('orders');
        ...
        console.warn('Invalid user data! Clearing for security.');
        logout();
        localStorage.removeItem('current-user');
        localStorage.removeItem('user-session-timestamp');
        localStorage.removeItem('orders');
      ...
      console.warn('State mismatch: currentUser exists but no localStorage data. Clearing state.');
      logout();
    }
