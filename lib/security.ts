...
// During build, Next.js sets NODE_ENV=production but we're not actually running the app
const isBuildTime = process.env.NEXT_PHASE === 'phase-production-build' || 
                    process.env.NEXT_PHASE === 'phase-development-build';

if (!ENCRYPTION_KEY && !isBuildTime) {
  if (process.env.NODE_ENV === 'production') {
    // Only throw at runtime in production, not during build
    throw new Error('NEXT_PUBLIC_ENCRYPTION_KEY environment variable is required in production');
  }
  // In development, use a warning but allow fallback for testing
  console.warn('⚠️ WARNING: NEXT_PUBLIC_ENCRYPTION_KEY not set. Using default key for development only.');
}

const ENCRYPTION_KEY_FINAL = ENCRYPTION_KEY || 'default-key-change-in-production-dev-only';
