...
export const useCurrencyStore = create<CurrencyStore>()(
  persist(
    (set, get) => ({
      ...
      // Get fixed price for a product in the selected currency
      getPrice: (product: Product, currencyCode?: string) => {
        const code = currencyCode || get().selectedCurrency;
        
        // First, try to get fixed price for the currency
        if (product.prices && product.prices[code]) {
          return product.prices[code];
        }
        
        // If no fixed price, fall back to USD price
        ...
        try {
          ...
          if (data.success && data.rates) {
            ...
            Object.keys(updatedCurrencies).forEach(code => {
              if (data.rates[code]) {
                updatedCurrencies[code].rate = data.rates[code];
              }
            });
