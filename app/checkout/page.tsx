...
function CheckoutForm() {
      ...
      if (storedUser) {
        try {
          ...
          if (parsedUser.email !== currentUser.email) {
            console.warn('User mismatch in checkout! Redirecting to login.');
            router.push('/login');
            return;
          }
        } catch (e) {
          console.warn('Invalid user data in checkout! Redirecting to login.');
          router.push('/login');
          return;
        }
      }
  ...
  }, [currentUser, getAllOrders, extractAddressesFromOrders, getDefaultAddress]);
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-green-700 mt-0.5">
                          You save {isIndia ? '₹149' : <FormattedPrice price={baseShipping} />} on shipping with Card/UPI payment
                        </p>
                      </div>
                    </div>
                  </div>
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
            ...
            <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-4">
              ...
              <p className="text-sm text-green-700 mt-2">
                {activeCoupon.discountType === 'percentage'
                  ...
                  : <><FormattedPrice price={activeCoupon.discountValue} /> off</>}
              </p>
            </div>
          ) : (
            <div className="mb-4">
        ...
        <div className="space-y-4 mb-6">
          <div className="flex justify-between">
            <span>Subtotal</span>
            <span><FormattedPrice price={total} /></span>
          </div>
          <div className="flex justify-between">
            <span>Shipping</span>
            <span>
            ...
            <div className="bg-gradient-to-r from-green-50 to-emerald-50 border-2 border-green-400 rounded-lg p-3 mt-2">
              <div className="flex items-center space-x-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-green-700 mt-0.5">
                    You saved {isIndia ? '₹149' : <FormattedPrice price={baseShipping} />} on shipping by choosing Card/UPI payment
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="border-t pt-4 flex justify-between text-xl font-bold">
            <span>Total</span>
            <span><FormattedPrice price={finalTotal} /></span>
          </div>
        </div>
        <button
          type="submit"
        ...
        >
          {paymentMethod === 'card' ? (
            <>
              ...
              <span>{loading ? 'Processing...' : <>Pay <FormattedPrice price={finalTotal} /></>}</span>
            </>
          ) : paymentMethod === 'upi' ? (
            <>
              <Smartphone className="w-5 h-5" />
              <span>{loading ? 'Placing Order...' : <>Place Order - Pay <FormattedPrice price={finalTotal} /> via UPI</>}</span>
            </>
          ) : (
            <>
              <Banknote className="w-5 h-5" />
              <span>{loading ? 'Placing Order...' : <>Place Order - Pay <FormattedPrice price={finalTotal} /> on Delivery</>}</span>
            </>
          )}
        </button>
